...
class App {
          ...
          const imageURL = await snapshot.ref.getDownloadURL();
            ...
            uploadTask.on(
              ...
              (snapshot) => {
                  ...
                  this.$uploadProgress.innerText = `Upload Progress: ${progress.toFixed(2)}%`
              },
              ...
              () => {
                  ...
                  uploadTask.snapshot.ref.getDownloadURL().then((downloadURL) => {
                      // You can now use the downloadURL to display the uploaded image or save it to a database.
                      console.log('Image uploaded:', downloadURL)
                      this.$uploadProgress.innerText = 'Upload Complete'
                  });
              }
          );
  
          // Create a post document in Firestore
          this.userId = firebase.auth().currentUser.uid; // Get the user's UID

          const postRef = db.collection('posts').doc(this.userId)
  ...
  fetchPostsFromDB() {
    var docRef = db.collection("posts").doc(this.userId);
    ...
    docRef.get().then((doc) => {
        if (doc.exists) {
            ...
            db.collection("posts").doc(this.userId).set({
                posts: []
            })
                .then(() => {
...
render() {
  ...
  />
    ...
    <div class="footer">
      ...
      >
      <span class="caption">
        <span class="caption-username"><b>${this.user.displayName}</b></span>
